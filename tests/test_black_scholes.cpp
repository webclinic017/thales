...
TEST_F(BlackScholesTest, CalculateOptionPrice) {
    ...
    double price =
        BlackScholes::calculate_option_price(S, K, T, r, sigma, type);
...
TEST_F(BlackScholesTest, InvalidOptionType) {
    EXPECT_THROW(BlackScholes::calculate_option_price(
                     100.0, 100.0, 1.0, 0.05, 0.2, static_cast<OptionType>(-1)),
                 std::invalid_argument);
}

TEST_F(BlackScholesTest, InvalidInputParameters) {
    EXPECT_THROW(BlackScholes::calculate_option_price(-100.0, 100.0, 1.0, 0.05,
                                                      0.2, OptionType::CALL),
                 std::invalid_argument);
    EXPECT_THROW(BlackScholes::calculate_option_price(100.0, -100.0, 1.0, 0.05,
                                                      0.2, OptionType::CALL),
